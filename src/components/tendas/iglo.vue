  ...
  <v-container>
    ...
    <v-stepper v-model="mySlider">
        ...
        <v-stepper-content step="2">
            ...
            <v-card-text>
            <v-container>
                <v-form v-model="form_cobertura" ref="form_cobertura">
                    <v-layout row wrap>
                    ...
                    <v-flex xs6 sm9>
                        <v-text-field label="Transparente" readonly :rules="maxCobertura" v-model="tenda.cobertura_transparent" type="number"></v-text-field>
                    </v-flex>
                    <v-flex xs3 sm1 text-xs-right>
                        <v-btn @click.native="tenda.cobertura_transparent = tenda.cobertura_transparent+1" color="dark" fab small >
                            <v-icon >add</v-icon>
                        </v-btn>
                    </v-flex>
                    <v-flex xs3 sm1 text-xs-right>
                        <v-btn @click.native="tenda.cobertura_transparent > 0 ? tenda.cobertura_transparent-- : tenda.cobertura_transparent = 0" color="dark" fab small >
                            <v-icon >remove</v-icon>
                        </v-btn>
                    </v-flex>
...
} from "@/utils/tendas/iglo.js";
import jsPDF from "jspdf";
require("jspdf-autotable");
export default {
  data() {
    return {
      tamanho_tendas: [["10", "15", "20", "25"]],
      tenda: {
        tipo: "",
        largura: "",
        comprimento: "",
      ...
      maxCobertura: [
          v => this.tenda.cobertura_opaco+this.tenda.cobertura_transparent+this.tenda.cobertura_blackout <= this.max_cobertura || "Demasiadas Coberturas"
      ],
      maxLaterais: [
  ...
  methods: {
    ...
    estrutura_iglo: function() {
      if (this.$refs.form_estrutura.validate()) {
        ...
        if (
          this.tenda.tipo_topo_2 == "Redondo" ||
          this.tenda.tipo_topo_1 == "Redondo"
        ) {
        ...
        if (
          this.tenda.comprimento == this.tenda.largura &&
          (this.tenda.tipo_topo_1 == "Redondo" &&
            this.tenda.tipo_topo_2 == "Redondo")
        )
          return;
    ...
    generatePDF() {
      if (this.$refs.info_form.validate()) {
        ...
        doc.text(
          100,
          120,
          "Dimensões - " + this.tenda.largura + "x" + this.tenda.comprimento
        );
        ...
        if (this.tenda.tipo_topo_1 != this.tenda.tipo_topo_2) {
          doc.text(
            100,
            140,
            "Topo - " + this.tenda.tipo_topo_1 + " e " + this.tenda.tipo_topo_2
          );
        } else {
